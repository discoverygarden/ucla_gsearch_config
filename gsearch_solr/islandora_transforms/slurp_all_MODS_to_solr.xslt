...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="mods:mods">
    ...
    <xsl:for-each select=".//mods:*[not(@type='date')][not(contains(translate(local-name(), 'D', 'd'), 'date')) or (self::mods:dateCreated)][normalize-space(text())]">
      <field>
    ...
    <!-- Handle dates. -->
    <xsl:for-each select=".//mods:*[(@type='date') or (contains(translate(local-name(), 'D', 'd'), 'date'))][not(self::mods:dateCreated)][normalize-space(text())]">
